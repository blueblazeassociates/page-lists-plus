...
function page_lists_plus_install() {
	...
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD show_in_menu TINYINT(1) DEFAULT 1 NOT NULL AFTER post_title');
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD link_link TINYINT(1) DEFAULT 1 NOT NULL AFTER show_in_menu');
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD no_follow_link TINYINT(1) DEFAULT 0 NOT NULL AFTER link_link');
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD alt_link_text VARCHAR(250) AFTER no_follow_link');
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD custom_link_class VARCHAR(50) AFTER alt_link_text');
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD redirect_url VARCHAR(100) AFTER custom_link_class');
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD target_blank TINYINT(1) DEFAULT 0 NOT NULL AFTER redirect_url');
	$wpdb->query('ALTER TABLE ' . $posts_table . ' ADD alt_title_attribute VARCHAR(250) AFTER target_blank');
...
function page_lists_plus_uninstall() {
	...
	if (get_option('plp_delete_data_on_deactivation') == 'on') {
		...
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP show_in_menu');
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP link_link');
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP no_follow_link');
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP alt_link_text');
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP custom_link_class');
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP redirect_url');
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP target_blank');
		$wpdb->query('ALTER TABLE ' . $posts_table . ' DROP alt_title_attribute');
...
function add_page_lists_plus_admin_menu() {
	add_options_page('Page Lists Plus', 'Page Lists Plus', 'level_9', __FILE__, 'page_lists_plus_admin_menu');
}
...
function save_page_lists_plus() {
	...
	if ( isset( $_POST['manual_save'] ) && $_POST['manual_save'] == 'manual_save') {

		if ($_POST['alt_link_text'] == '') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET alt_link_text = null WHERE ID = ' . $_POST['ID']);
		} elseif (get_option('plp_show_link_text_field') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET alt_link_text = \'' . $_POST['alt_link_text'] . '\' WHERE ID = ' . $_POST['ID']);
		}

		if ($_POST['alt_title_attribute'] == '') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET alt_title_attribute = null WHERE ID = ' . $_POST['ID']);
		} elseif (get_option('plp_show_title_attribute_field') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET alt_title_attribute = \'' . $_POST['alt_title_attribute'] . '\' WHERE ID = ' . $_POST['ID']);
		}

		if ($_POST['redirect_url'] == '') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET redirect_url = null WHERE ID = ' . $_POST['ID']);
		} elseif (get_option('plp_show_redirect_field') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET redirect_url = \'' . $_POST['redirect_url'] . '\' WHERE ID = ' . $_POST['ID']);
		}

		if ($_POST['custom_link_class'] == '') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET custom_link_class = null WHERE ID = ' . $_POST['ID']);
		} elseif (get_option('plp_show_link_class_field') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET custom_link_class = \'' . $_POST['custom_link_class'] . '\' WHERE ID = ' . $_POST['ID']);
		}

		if ($_POST['target_blank']) {
			$wpdb->query('UPDATE ' . $posts_table . ' SET target_blank = \'1\' WHERE ID = ' . $_POST['ID']);
		} elseif (get_option('plp_show_target_blank') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET target_blank = \'0\' WHERE ID = ' . $_POST['ID']);
		}

		if ($_POST['show_in_menu']) {
			$wpdb->query('UPDATE ' . $posts_table . ' SET show_in_menu = \'1\' WHERE ID = ' . $_POST['ID']);

			// Include descendants
			if (get_option('plp_include_children') == 'on') {
				$descendants = get_pages('child_of='. $_POST['ID']);
				foreach($descendants as $descendant) {
					$wpdb->query('UPDATE ' . $posts_table . ' SET show_in_menu = \'1\' WHERE ID = ' . $descendant->ID);
				}
			}
		} elseif (get_option('plp_show_include_field') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET show_in_menu = \'0\' WHERE ID = ' . $_POST['ID']);

			// Exclude descendants
			if (get_option('plp_exclude_children') == 'on') {
				$descendants = get_pages('child_of='. $_POST['ID']);
				foreach($descendants as $descendant) {
					$wpdb->query('UPDATE ' . $posts_table . ' SET show_in_menu = \'0\' WHERE ID = ' . $descendant->ID);
				}
			}
		}

		if ($_POST['link_link']) {
			$wpdb->query('UPDATE ' . $posts_table . ' SET link_link = \'1\' WHERE ID = ' . $_POST['ID']);
		} elseif (get_option('plp_show_link_field') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET link_link = \'0\' WHERE ID = ' . $_POST['ID']);
		}

		if ($_POST['no_follow_link']) {
			$wpdb->query('UPDATE ' . $posts_table . ' SET no_follow_link = \'1\' WHERE ID = ' . $_POST['ID']);
		} elseif (get_option('plp_show_nofollow_field') == 'on') {
			$wpdb->query('UPDATE ' . $posts_table . ' SET no_follow_link = \'0\' WHERE ID = ' . $_POST['ID']);
		}
...
function page_exclusions($page_exclusions) {
	global $wpdb;
	$posts_table = $wpdb->prefix . 'posts';
	$page_exlusions_data = $wpdb->get_results('SELECT ID FROM ' . $posts_table . ' WHERE show_in_menu = \'0\' AND post_status = \'publish\'',ARRAY_A);
	foreach ($page_exlusions_data as $row) {
		extract($row);
		$page_exclusions[] = $ID;
...
function page_lists_plus($output) {
	...
	$alt_title_attribute_data = $wpdb->get_results('SELECT post_title, alt_title_attribute FROM ' . $posts_table . ' WHERE alt_title_attribute IS NOT NULL AND post_status = \'publish\'',ARRAY_A);
	foreach ($alt_title_attribute_data as $row) {
		extract($row);
	...
	$no_follow_link_data = $wpdb->get_results('SELECT post_title, no_follow_link FROM ' . $posts_table . ' WHERE no_follow_link = \'1\' AND post_status = \'publish\'',ARRAY_A);
	foreach ($no_follow_link_data as $row) {
		extract($row);
		$post_title = qTrans_compatibility(wptexturize($post_title));
		$output = str_replace('>' . $post_title . '<', ' rel="nofollow">' . $post_title . '<', $output);
	}

	$target_blank_data = $wpdb->get_results('SELECT post_title, target_blank FROM ' . $posts_table . ' WHERE target_blank = \'1\' AND post_status = \'publish\'',ARRAY_A);
	foreach ($target_blank_data as $row) {
		extract($row);
		$post_title = qTrans_compatibility(wptexturize($post_title));
		$output = str_replace('>' . $post_title . '<', ' target="_blank">' . $post_title . '<', $output);
	}

	$link_link_data = $wpdb->get_results('SELECT post_title FROM ' . $posts_table . ' WHERE link_link = \'0\' AND post_status = \'publish\'',ARRAY_A);
	foreach ($link_link_data as $row) {
		extract($row);
		$post_title = qTrans_compatibility(wptexturize($post_title));
	...
	if (get_option('plp_unlink_current') == 'on') {
		global $post;
		$current_page_data = $wpdb->get_results('SELECT post_title FROM ' . $posts_table . ' WHERE ID = \'' . $post->ID . '\' AND post_status = \'publish\'',ARRAY_A);
	...
	if (get_option('plp_add_spans_at_start_of_list_items') == 'on') {
		...
		if (get_option('plp_unlink_current') == 'on' && get_option('plp_unlink_js') != 'on') {
			global $post;
			$current_page_data = $wpdb->get_results('SELECT post_title FROM ' . $posts_table . ' WHERE ID = \'' . $post->ID . '\' AND post_type = \'page\'',ARRAY_A);
			foreach ( $current_page_data as $row ) {
				extract($row);
				$post_title = qTrans_compatibility(wptexturize($post_title));
				$output = str_replace($post_title, '<span class="gilder-levin"></span>' . $post_title, $output);
	...
	$alt_link_text_data = $wpdb->get_results('SELECT post_title, alt_link_text FROM ' . $posts_table . ' WHERE alt_link_text IS NOT NULL AND post_status = \'publish\'',ARRAY_A);
	foreach ( $alt_link_text_data as $row ) {
		extract($row);
		$post_title = qTrans_compatibility(wptexturize($post_title));
		$output = str_replace('>' . $post_title . '<', '>' . $alt_link_text . '<', $output);
	}

	$custom_link_class_data = $wpdb->get_results('SELECT ID, custom_link_class FROM ' . $posts_table . ' WHERE custom_link_class IS NOT NULL AND post_status = \'publish\'',ARRAY_A);
	foreach ( $custom_link_class_data as $row ) {
		extract($row);
	...
	$redirect_url_data = $wpdb->get_results('SELECT ID, redirect_url FROM ' . $posts_table . ' WHERE redirect_url IS NOT NULL AND post_status = \'publish\'',ARRAY_A);
	foreach ( $redirect_url_data as $row ) {
		extract($row);
